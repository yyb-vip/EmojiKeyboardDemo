...
@interface YBEmojiGifImageView ()

@property (nonatomic, strong) YBEmojiGifImage *animatedImage;
...
@implementation YBEmojiGifImageView
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([image isKindOfClass:[YBEmojiGifImage class]] && image.images) {
        if([image.images[0] isKindOfClass:UIImage.class])
            [super setImage:image.images[0]];
        else
            [super setImage:nil];
        self.currentFrame = nil;
        self.animatedImage = (YBEmojiGifImage *)image;
...
- (void)setAnimatedImage:(YBEmojiGifImage *)animatedImage
{
